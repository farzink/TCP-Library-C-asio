...
public:
	...
	void success(string text, bool newline = true) {
		if (newline)
			cout << termcolor::green << text << std::endl << termcolor::reset;
		else
			cout << termcolor::green << text << termcolor::reset;
	}
	void failure(string text, bool newline = true) {
		if (newline)
			cout << termcolor::red << text << std::endl << termcolor::reset;
		else
			cout << termcolor::red << text << termcolor::reset;
	}
};
#endif
